...
const TIER: Record<string, { tone: string; ring: string }> = {
  ...
  None: { tone: "text-gray-400", ring: "border-white/10" },
};
    ...
    <section className="card">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h2 className="flex items-center gap-2 text-lg font-semibold">
          <Radar className="h-5 w-5 text-indigo-400" /> Coordinated Inauthentic Behavior
        </h2>
        {!r && (
          <button onClick={run} disabled={loading}
            className="flex items-center gap-2 rounded-lg bg-indigo-600 px-3 py-1.5 text-sm font-semibold text-white transition hover:bg-indigo-500 disabled:opacity-50">
        ...
        <div className="mt-4 space-y-4">
          <div className={`rounded-xl border ${tier.ring} bg-white/[0.02] p-3`}>
            <div className="flex flex-wrap items-baseline gap-2">
              <span className="text-xs uppercase tracking-wide text-gray-500">Coordination Likelihood</span>
              <span className={`text-lg font-bold ${tier.tone}`}>{r.likelihood}</span>
              <span className="text-sm text-gray-500">· actor <strong className="text-gray-300">UNDETERMINED</strong></span>
            </div>
            <div className="mt-0.5 text-xs text-gray-500">{r.totalItems} public mentions · {r.accounts} accounts</div>
          </div>
            ...
            {r.signals.map((s: any, i: number) => (
              <div key={i}>
                ...
                {s.alternative && s.alternative !== "n/a" && !s.alternative.startsWith("n/a") && (
                  <p className="mt-0.5 text-xs text-gray-500"><span className="text-gray-600">Could also be:</span> {s.alternative}</p>
                )}
              </div>
            ))}
          </div>
          ...
          {r.clusters?.length > 0 && (
            <div>
              <div className="text-xs font-semibold text-gray-400">Near-identical content clusters</div>
              <div className="mt-1 space-y-1">
                {r.clusters.slice(0, 5).map((c: any, i: number) => (
                  <div key={i} className="rounded-lg border border-white/[0.06] px-3 py-1.5 text-xs">
                    <span className="text-gray-300">{c.size} posts · {c.accounts} accounts · {c.sources.join(", ")}</span>
                    <div className="text-gray-500">“{c.text}”</div>
                  </div>
                ))}
              </div>
            </div>
