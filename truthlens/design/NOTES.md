...
## Copy rule (enforced)
**No em-dash (`—`, U+2014) anywhere in the system - ever.** Use a hyphen, comma,
colon, or period instead. Applies to UI copy, comments, and data strings. (En-dash
`–` for numeric ranges like 0-35 and arrows `→` are fine.) A grep guard should flag
any `—` reintroduced.
