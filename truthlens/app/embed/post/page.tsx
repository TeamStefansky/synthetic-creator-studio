...
function color(v: PostVerdict) {
  switch (v) {
    case "Likely False": return "#fb7185";
    case "Misleading": return "#fbbf24";
    case "Likely True": return "#34d399";
    case "Opinion or Satire": return "#818cf8";
...
function EmbedInner() {
  ...
  if (err) return <div className="p-4 text-sm text-gray-400">{err}</div>;
  if (!res) return <div className="p-4 text-sm text-gray-400">Loading…</div>;

  return (
    <div className="mx-auto max-w-md p-3">
      <div className="rounded-xl border border-white/10 bg-bg-card p-4">
        <div className="mb-2 flex items-center gap-2">
          <span className="grid h-6 w-6 place-items-center rounded-md bg-gradient-to-br from-indigo-500 to-violet-600 text-[11px] font-bold text-white">T</span>
          <span className="text-xs font-medium text-gray-400">TruthLens · Post Check</span>
        </div>
        <div className="text-lg font-bold" style={{ color: color(res.verdict) }}>{res.verdict}</div>
        <div className="mb-2 text-xs text-gray-500">Confidence: {res.confidence}</div>
        <p className="text-sm text-gray-200">{res.summary}</p>
        {res.sources.length > 0 && (
          <a href={res.sources[0].url} target="_blank" rel="noreferrer" className="mt-2 inline-block text-xs text-indigo-400 hover:underline">
            {res.sources.length} source(s) →
          </a>
        )}
        <div className="mt-3 border-t border-white/10 pt-2 text-[11px] text-gray-500">
