...
class Settings(BaseSettings):
    ...
    # gdelt/bluesky/hackernews/reddit/mastodon are free & keyless (real search,
    # no signup); the rest activate when their (mostly free) key is set.
    enabled_sources: str = (
        "x,telegram,rss,newsapi,gdelt,bluesky,hackernews,reddit,mastodon,"
        "guardian,nyt,gnews,newsdata,mediastack,brave,youtube"
    )

    # Optional real-source credentials (source is inert/mock when its key absent).
    ...
    # Free-key news / video / search sources.
    ...
    # Mastodon instance whose public timeline is read (keyless).
